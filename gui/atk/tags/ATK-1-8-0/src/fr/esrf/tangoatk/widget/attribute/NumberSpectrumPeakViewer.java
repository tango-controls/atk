                         ...
                         implements fr.esrf.tangoatk.core.INumberScalarListener
{
  ...
  private void changeAffineTransform()
  {
      ...
      // compute afine transform for ref curve
      if (refCurveDV == null)
         return;
	 
      refCurve_nb_x_values = refCurveDV.getDataLength();
      if (refCurve_nb_x_values <= 0)
         return;
      
      refCurve_step = (max_x - min_x) / refCurve_nb_x_values;
      refCurveA0 = min_x;
...
//System.out.println("firstIndex = "+firstIndex + "  plotIndex = " + plotIndex + " et  lastIndex = " + lastIndex);
  ...
  /**
   * Overloads the same methon in spectrumViewer class
   *
   */
  ...
  public void spectrumChange(NumberSpectrumEvent numberSpectrumEvent)
  {
      ...
      // Compute the new affine transform of the spectrum based on the new length
      step = (max_x - min_x) / nb_x_values;
      setXAxisAffineTransform(min_x, step);
      
      dvy.reset();
      for (int i = 0; i < nb_x_values; i++)
      {
	  dvy.add(A0 + A1 * (double) i, value[i]);
      }
      
      // compute afine transform for ref curve
      if (refCurveValues != null)
      {
	 nb_x_values = refCurveValues.length;
	 if (nb_x_values > 0)
	 {
	       step = (max_x - min_x) / nb_x_values;
	       refCurveA0 = min_x;
	       ...
	       for (int i = 0; i < nb_x_values; i++)
	       {
        	   refCurveDV.add(refCurveA0 + refCurveA1 * (double) i, refCurveValues[i]);
	       }
	 }
      }
  ...
  public void updateRefCurve(double[]  values)
  {
