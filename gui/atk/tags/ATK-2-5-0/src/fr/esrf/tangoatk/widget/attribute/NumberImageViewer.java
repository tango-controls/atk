...
public class NumberImageViewer extends JPanel implements IImageListener, MouseMotionListener, MouseListener, ActionListener, KeyListener, JDrawable {
  ...
  private static int defaultDividerLocation = 60;
  ...
  public NumberImageViewer() {
    ...
    buttonPanel.setPreferredSize(new Dimension(40, 340));
    ...
    separator.setDividerLocation(defaultDividerLocation);
    separator.setLeftComponent(buttonView);
    separator.setContinuousLayout(true);
    separator.setPreferredSize(new Dimension(60+imagePanel.getWidth(), imagePanel.getHeight()));
...
/*-- Layout formating --*/
    SpringLayout layout = (SpringLayout)buttonPanel.getLayout();
    //Align all cells in each row and make them the same width.
    Spring x = Spring.constant(0);
    Spring width = Spring.constant(0);
    Spring height = Spring.constant(0);
    Spring y = Spring.constant(0);
    for (int r = 0; r < buttonPanel.getComponents().length; r++) {
        Constraints cons = ((SpringLayout)buttonPanel.getLayout()).getConstraints(buttonPanel.getComponent(r));
        width = Spring.max(width,cons.getWidth());
        height = Spring.max(height,cons.getHeight());
    }
    for (int r = 0; r < buttonPanel.getComponents().length; r++) {
        SpringLayout.Constraints constraints =
            ((SpringLayout)buttonPanel.getLayout()).getConstraints(buttonPanel.getComponent(r));
        constraints.setX(x);
        constraints.setWidth(width);
        constraints.setY(y);
        constraints.setHeight(height);
        y = Spring.sum(y, Spring.sum(height, Spring.constant(0)));
    }
    //Set the buttonPanel's size.
    SpringLayout.Constraints pCons = layout.getConstraints(buttonPanel);
    pCons.setConstraint(SpringLayout.SOUTH, y);
    pCons.setConstraint(SpringLayout.EAST, x);
/*-- end of Layout formating --*/
  ...
  private void updatePreferredSize() {
    if (imagePanel == null || cfgPanel == null || gradientTool == null || separator == null) {
      return;
    }
    int pWidth = gradientTool.getPreferredSize().width + 1;
    int pHeight = cfgPanel.getPreferredSize().height + 10;
    pWidth += imagePanel.getPreferredSize().width + separator.getDividerLocation();
    if (imagePanel.getPreferredSize().height >= buttonPanel.getPreferredSize().height) {
      pHeight += imagePanel.getPreferredSize().height;
    }
    else {
      pHeight += buttonPanel.getPreferredSize().height;
    }
    separator.setPreferredSize(new Dimension(pWidth,pHeight));
  }
  ...
  public void setToolbarVisible(boolean b) {
    if (separator != null) {
      if (b) {
        ...
        separator.setDividerLocation(0);
      }
    }
    ...
    if (evt.getSource() == selectButton ||
      ...
      FileFilter edfFilter = new FileFilter() {
        ...
        public String getDescription() {
          return "edf - EDF 16Bits snapshot";
        }
      };
        ...
        if (f != null) {
          ...
          if (ok == JOptionPane.YES_OPTION){
            if (getExtension(f)==null){
              JOptionPane.showMessageDialog(this,"Unknown file type","Error",JOptionPane.ERROR_MESSAGE);
            }
            else if (getExtension(f).equalsIgnoreCase("edf")){
...
//      INumberImage theAtt = (INumberImage) attributeList.add("sy/d-tm/profile/TuneSpectra");
...
//      INumberImage theAtt = (INumberImage) attributeList.add("//athena:20000/id14/eh3-camera/sony/image");
    ...
    d.addCrossMarker(512, 512, Color.GREEN);
