...
// Horizontal autoScale
  ...
  // Expert usage
  private int paintYOutTicks(Graphics g, int x0, int ys, int length, int y0, int la, BasicStroke bs, int step, int tr,int off,boolean grid) {
      ...
      if (step == -1) {
        ...
        for (j = 0; j < logStep.length; j++) {
          ...
          if (h > y0 && h < (y0 + csize.height)) {
            g.drawLine(x0 + tr + off , h, x0 + tr + off + 2, h);
  ...
  // Paint sub tick outside label limit
  // Expert usage
  private int paintXOutTicks(Graphics g, int y0, int xs, int length, int x0, int la, BasicStroke bs, int step, int tr,int off,boolean grid) {
      ...
      if (step == -1) {
        ...
        for (j = 0; j < logStep.length; j++) {
          ...
          if (w > x0 && w < (x0 + csize.width)) {
            g.drawLine(w, y0 + tr + off, w, y0 + tr + off + 2);
            if (gridVisible && subGridVisible && grid) {
              if (bs != null) g2.setStroke(bs);
  ...
  // Expert usage
  private int paintYTicks(Graphics g, int i, int x0, int y, int length, int la, BasicStroke bs, int tr,int off,boolean grid) {
  ...
  // Expert usage
  private int paintXTicks(Graphics g, int i, int y0, int x, int length, int la, BasicStroke bs, int tr,int off,boolean grid) {
  ...
  public void paintAxisDirect(Graphics g, FontRenderContext frc, int x0, int y0,Color back,int tr,int la) {
    ...
    tickOff = getTickOffset(4);
    subTickOff = getTickOffset(2);
    ...
    switch (dOrientation) {
      ...
      case VERTICAL_LEFT:
        ...
        for (i = 0; i < labels.size(); i++) {
          ...
          if (i==(labels.size() - 1) && tickStep != 0) {
            LabelInfo lis = (LabelInfo) labels.get(0);
            LabelInfo lie = (LabelInfo) labels.get(i);

            g.setColor(subgridColor);
            paintYOutTicks(g, x0 + csize.width, y0 + lis.pos - tickStep, tickStep, y0, la, bs, lis.subtick_step, tr,subTickOff,true);
            paintYOutTicks(g, x0 + csize.width, y0 + lie.pos, tickStep, y0, la, bs, lis.subtick_step, tr,subTickOff,true);
          }

          //Draw sub tick
          g.setColor(subgridColor);
          int ts = paintYTicks(g, i, x0 + csize.width, y, -li.pos, la, bs, tr,subTickOff,true);
          if (ts != 0 && tickStep == 0) tickStep = ts;

          //Draw tick
          g.setColor(labelColor);
          g.drawLine(x0 + tr + csize.width + tickOff, y, x0 + tr + csize.width + tickOff + 4, y);
      ...
      case VERTICAL_RIGHT:
        ...
        for (i = 0; i < labels.size(); i++) {
          ...
          //Draw sub tick
          g.setColor(subgridColor);
          int ts = paintYTicks(g, i, x0, y, -li.pos, -la, bs, tr,subTickOff,true);
          if (ts != 0 && tickStep == 0) tickStep = ts;
          ...
          if (i==(labels.size() - 1) && tickStep != 0) {
            ...
            g.setColor(subgridColor);
            paintYOutTicks(g, x0, y0 + lis.pos - tickStep, tickStep, y0, -la, bs, lis.subtick_step, tr,subTickOff,true);
            paintYOutTicks(g, x0, y0 + lie.pos, tickStep, y0, -la, bs, lis.subtick_step, tr,subTickOff,true);
          }

          //Draw tick
          g.setColor(labelColor);
          g.drawLine(x0 + tr + tickOff , y, x0 + tr + tickOff + 4, y);
        }
      ...
      case HORIZONTAL_DOWN:
        ...
        for (i = 0; i < labels.size(); i++) {
          ...
          //Draw sub tick
          g.setColor(subgridColor);
          int ts = paintXTicks(g, i, y, x, -li.pos, -la, bs, tr,subTickOff,true);
          if (ts != 0 && tickStep == 0) tickStep = ts;
          ...
          if (i==(labels.size() - 1) && tickStep != 0) {
            ...
            g.setColor(subgridColor);
            paintXOutTicks(g, y0, x0 + lis.pos - tickStep, tickStep, x0, -la, bs, lis.subtick_step, tr,subTickOff,true);
            paintXOutTicks(g, y0, x0 + lie.pos, tickStep, x0, -la, bs, lis.subtick_step, tr,subTickOff,true);
          }
          ...
          //Draw tick
          g.setColor(labelColor);
          g.drawLine(x, y0 + tr + tickOff, x, y0 + tr + tickOff + 4);
  ...
  public void paintAxisOpposite(Graphics g, FontRenderContext frc, int x0, int y0,Color back,int tr,int la) {
    ...
    tickOff = getTickOffsetOpposite(4);
    subTickOff = getTickOffsetOpposite(2);
    ...
    switch (dOrientation) {
      ...
      case VERTICAL_RIGHT:
        ...
        for (i = 0; i < labels.size(); i++) {
          ...
          //Draw sub tick
          g.setColor(subgridColor);
          int ts = paintYTicks(g, i, nX0 + csize.width, y, -li.pos, la, bs, tr,subTickOff,false);
          if (ts != 0 && tickStep == 0) tickStep = ts;
          ...
          if (i==(labels.size() - 1) && tickStep != 0) {
            ...
            g.setColor(subgridColor);
            paintYOutTicks(g, nX0 + csize.width, y0 + lis.pos - tickStep, tickStep, y0, la, bs, lis.subtick_step, tr,subTickOff,false);
            paintYOutTicks(g, nX0 + csize.width, y0 + lie.pos, tickStep, y0, la, bs, lis.subtick_step, tr,subTickOff,false);
          }

          //Draw tick
          g.setColor(labelColor);
          g.drawLine(nX0 + tr + csize.width + tickOff, y, nX0 + tr + csize.width + tickOff + 4, y);
      ...
      case VERTICAL_LEFT:
        ...
        for (i = 0; i < labels.size(); i++) {
          ...
          //Draw sub tick
          g.setColor(subgridColor);
          int ts = paintYTicks(g, i, x, y, -li.pos, -la, bs, tr,subTickOff,false);
          if (ts != 0 && tickStep == 0) tickStep = ts;
          ...
          if (i==(labels.size() - 1) && tickStep != 0) {
            ...
            g.setColor(subgridColor);
            paintYOutTicks(g, x, y0 + lis.pos - tickStep, tickStep, y0, -la, bs, lis.subtick_step, tr,subTickOff,false);
            paintYOutTicks(g, x, y0 + lie.pos, tickStep, y0, -la, bs, lis.subtick_step, tr,subTickOff,false);
          }

          //Draw tick
          g.setColor(labelColor);
          g.drawLine(x + tr + tickOff , y, x + tr + tickOff + 4, y);
        }
      ...
      case HORIZONTAL_DOWN:
        ...
        for (i = 0; i < labels.size(); i++) {
          ...
          //Draw sub tick
          g.setColor(subgridColor);
          int ts = paintXTicks(g, i, y, x, -li.pos, -la, bs, tr,subTickOff,false);
          if (ts != 0 && tickStep == 0) tickStep = ts;
          ...
          if (i==(labels.size()-1) && tickStep!= 0) {
            ...
            g.setColor(subgridColor);
            paintXOutTicks(g, y0, x0 + lis.pos - tickStep, tickStep, x0, -la, bs, lis.subtick_step, tr,subTickOff,false);
            paintXOutTicks(g, y0, x0 + lie.pos, tickStep, x0, -la, bs, lis.subtick_step, tr,subTickOff,false);
          }
