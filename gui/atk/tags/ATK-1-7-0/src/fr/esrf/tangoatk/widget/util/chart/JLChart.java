...
class TabbedLine {
  ...
  String[] getNextFields() {
    ...
    for (int i = 0; i < dl.length; i++) {
      if (dl[i] != null) {
        if (dl[i].x == t0) {
          ret[i + sIndex] = Double.toString(dl[i].y);
          dl[i] = dl[i].next;
        } else {
          ret[i + sIndex] = "";
        }
...
public class JLChart extends JComponent implements MouseListener, MouseMotionListener, ActionListener {
  ...
  public JLChart() {
    ...
    tableMenuItem = new JMenuItem("Show table");
  ...
  // Display a JTable containing data
  ...
  public void actionPerformed(ActionEvent evt) {
    ...
    if (evt.getSource() == optionMenuItem) {
      ...
      exitZoom();
    } else if (evt.getSource() == tableMenuItem) {
      showTable();
    } else if (evt.getSource() == fileMenuItem) {
  ...
  private void paintLabelAndHeader(Graphics2D g, FontRenderContext frc) {
    ...
    if (headerR.width>0) {
      ...
      xpos = (int) ((headerR.width - headerWidth) / 2);
    ...
    if (labelR.width>0) {
      ...
      for (i = 0; i < nbv1; i++) {
        ...
        y = ypos + labelSHeight * i + labelSHeight / 2;
      ...
      for (int j = 0; j < nbv2; j++) {
        ...
        g.setColor(v.getColor());
        y = ypos + labelSHeight * (i+j) + labelSHeight / 2;
        JLAxis.drawSampleLine(g, xpos, y - 2, v);
  ...
  private void measureGraphItems(Graphics2D g, FontRenderContext frc, int w, int h, Vector views) {
    ...
    // Measure labels ------------------------------------------------------
    if (labelVisible && (nb>0) && (labelFont!=null)) {
      ...
      labelHeight = (int) (labelSHeight * nb);
    ...
    if (y1Axis.getViews().size() > 0) {
      axisFontUp = y1Axis.getFontHeight(g);
    } else if (y2Axis.getViews().size() > 0) {
    ...
    y1Axis.measureAxis(g, frc, 0, axisHeight);
    y2Axis.measureAxis(g, frc, 0, axisHeight);
    ...
    xAxis.measureAxis(g, frc, axisWidth, 0);
  ...
  protected void paintComponent(Graphics g) {
    ...
    paintLabelAndHeader(g2,frc);

    // Paint chart background
    int xOrg = viewR.x + y1AxisThickness;
    int yOrg = viewR.y + axisFontUp + axisHeight;
    ...
    if (paintAxisFirst) {
      ...
      if( xAxis.getPosition()==JLAxis.HORIZONTAL_ORG2)
        xAxis.paintAxis(g, frc, xOrg, yOrg, y2Axis, 0, 0, getBackground());
      else
  ...
  public static void main(String args[]) {
    ...
    // Add it to the chart
    chart.getY1Axis().addDataView(v2);
