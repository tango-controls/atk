...
public class TrendOptions extends javax.swing.JPanel implements IApplicable {
	...
	this();
	trend = t;
	setSamplingRate(t.getSamplingRate());
	setXAxisLength(t.getXAxisLength());

	setLogarithmicScale(t.isLogarithmicScale());
	buttonBar1.setControlee(this);
    }

    void setSamplingRate(double d) {
	samplingRate.setText(Double.toString(d));
    }

    void setXAxisLength(int length) {
	xAxisLength.setText(Integer.toString(length));
    }

    void setLogarithmicScale(boolean b) {
	logarithmic.setSelected(b);
    }
