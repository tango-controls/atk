                         ...
                         implements fr.esrf.tangoatk.core.INumberScalarListener
{
   ...
   * @param auto axis autoscaling state
  ...
  // -----------------------------------------------------------------
  // Overrides the errorChange of NumberSpectrumViewer to implement the
  // the errorChange for number scalars like peaks and min and max
  // -----------------------------------------------------------------
  ...
  public void errorChange(fr.esrf.tangoatk.core.ErrorEvent errorEvent)
  {
     ...
     if (errorEvent.getSource() == model) //source is the spectrum
     {
        super.errorChange(errorEvent);
	return;
     }
     
     peakIndex = peak_models.indexOf(errorEvent.getSource());
       
     if ( peakIndex >= 0 )
     {
	/* Do not remove the peakModel just because we received an error on the
	  eventually try to make it invisible
	removePeakModel((INumberScalar) errorEvent.getSource());
	 */
	peakDV = (JLDataView) peak_data_views.get(peakIndex);
	if (peakDV != null)
	{
	   peakDV.reset();
	}
  ...
  private void removePeakFromList(INumberScalar p)
  {
	...
	{
            obj = null;
	    obj = peak_data_views.get(peakIndex);
	    if (obj != null)
	    {
	       if (obj instanceof JLDataView)
	       {
...
//System.out.println("firstIndex = "+firstIndex + "  plotIndex = " + plotIndex + " et  lastIndex = " + lastIndex);
  ...
  public void displayRefCurve()
  {
       ...
       getY1Axis().addDataViewAt(0, refCurveDV); // put ref curve behind the main curve
  }
  ...
  public static void main(String[] args)
  {
       ...
       try
       {
          spect = (INumberSpectrum) attList.add("seglab/tuneserver2/1/Spectrum");
          minx = (INumberScalar) attList.add("seglab/tuneserver2/1/FreqMin");
          maxx = (INumberScalar) attList.add("seglab/tuneserver2/1/FreqMax");
          peakH = (INumberScalar) attList.add("seglab/tuneserver2/1/PeakHCenter");
          peakV = (INumberScalar) attList.add("seglab/tuneserver2/1/PeakVCenter");
          tSpan = (INumberScalar) attList.add("seglab/tuneserver2/1/TrackSpanWidth");
	  nspv.setModel(spect);
