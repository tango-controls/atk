...
public class CommandComboViewer extends javax.swing.JPanel {
    ...
    public CommandComboViewer() {
        initComponents();
	argFrame.getContentPane().add(scv);
    }
	...
	scv.setModel(command);
	scv.setDeviceButtonVisible(deviceButtonVisible);
	scv.setDescriptionVisible(descriptionVisible);
	scv.setInfoButtonVisible(infoButtonVisible);

	scv.setBorder(null);
	scv.setInputVisible(true);	     
	if (!command.takesInput()) {
	    scv.setInputVisible(false);
	    command.execute();
	} 
    ...
    SimpleCommandViewer scv = new SimpleCommandViewer();    
