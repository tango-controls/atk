...
// By:            <erik@assum.net>
// Time-stamp:    <2002-07-01 16:49:33, assum>
// 
// $Id$
// 
// Description:       
 ...
 */
public class Device implements IRefreshee, Serializable {
    ...
    protected String oldState, oldStatus;
    ...
    static {
	refreshers = new ThreadGroup("deviceRefreshers");
    }

    protected Device() {
	System.out.println("New device created...");
	...
	proxy = new DeviceProxy(name, host, port);
	init(name);
    }
...
// 	    build_connection();
	...
	throws DevFailed {
	return proxy.get_attribute_config(name);
    }
	...
	return proxy.get_attribute_config(AEntityFactory.extractEntityName(name));
    }
    ...
    /**
     * <code>setError</code> fires of an error event from this device
     *
	...
	log.error(s, t);
	log.debug("setError: firePropertyChange(\"error\", " + oldt + ", " + t
		  + ");");
    ...
    /**
     * <code>getState</code> returns the state of the device.
     *
	...
	DevState state = null;
	try {
	    return toString(proxy.state());
	} catch (DevFailed dev) {
	    setError("Couldn't read state: ", new ConnectionException(dev));
	} catch (Exception e) {
	    setError("Couldn't read state: ", e);
	}
	return "UNKNOWN";
    }
	...
	switch (state.value()) {
	case DevState._ON: return "ON";
	case DevState._OFF: return "OFF";
	case DevState._CLOSE: return "CLOSE";
	case DevState._OPEN: return "OPEN";
	case DevState._INSERT: return "INSERT";
	case DevState._EXTRACT: return "EXTRACT";
	case DevState._MOVING: return "MOVING";
	case DevState._STANDBY: return "STANDBY";
	case DevState._FAULT: return "FAULT";
	case DevState._INIT: return "INIT";
	case DevState._RUNNING: return "RUNNING";
	case DevState._ALARM: return "ALARM";
	case DevState._DISABLE: return "DISABLE";
	default:
	    return "UNKNOWN";	    
	} // end of switch ()
	...
	try {
	    ...
	    setError("Couldn't read status: ", new ConnectionException(e));
	    return "UNKNOWN";
	} 
    }
...
// 	try {
// 	    return new DeviceAttribute((proxy.read_attributes(names))[0]);
//	} catch (Exception e) {
...
//	} 
	...
	DeviceAttribute da;
	da = proxy.read_attribute(AEntityFactory.extractEntityName(name));
	return da;
    }
    ...
    throws DevFailed {
	//	System.out.println("proxy.command_inout(" + command + ", " + argin + ")");
	DeviceData data = proxy.command_inout(AEntityFactory.extractEntityName(command), argin);
	refresh();
	return data;
    }

    /**
     * <code>storeInfog</code>
     *
