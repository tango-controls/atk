...
// By:            <erik@assum.net>
// Time-stamp:    <2002-07-01 16:50:35, assum>
// 
// $Id$
// 
// Description:       
...
public class NumberSpectrum extends NumberImage
	...
	oldSpectrumValue =  new double[getMaxXDimension()];

	newSpectrumValue = new double[getMaxXDimension()];
    }
	...
	try {
 	    newSpectrumValue = numberHelper.
		getNumberSpectrumValue(readValueFromNetwork());
 	    numberHelper.fireSpectrumValueChanged(newSpectrumValue, timeStamp);
	} catch (DevFailed e) {
	    setError(e.getMessage(), new AttributeReadException(e));
	} catch (Exception e) {
	...
	try {
	    return numberHelper.getNumberSpectrumValue(readValueFromNetwork());
	} catch (DevFailed d) {
