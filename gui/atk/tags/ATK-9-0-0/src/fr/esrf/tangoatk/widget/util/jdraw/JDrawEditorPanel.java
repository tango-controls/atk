...
public class JDrawEditorPanel extends JPanel implements ActionListener,JDrawEditorListener,JDValueListener {
  ...
  /** 'Undo' toolbar button. */
  JButton  editToolUndoBtn;
  /** 'Redo' toolbar button. */
  JButton  editToolRedoBtn;
  /** 'Cut' toolbar button. */
  JButton  editToolCutBtn;
  /** 'Copy' toolbar button. */
  JButton  editToolCopyBtn;
  /** 'Paste' toolbar button. */
  JButton  editToolPasteBtn;
  /** 'Zoom In' toolbar button. */
  JButton  editToolZoomInBtn;
  /** 'Zoom Out' toolbar button. */
  JButton  editToolZoomOutBtn;
  /** 'Zoom percent' toolbar label. */
  JLabel   editToolZoomLabel;
  /** 'Object properties' toolbar button. */
  JButton  editToolOptionBtn;
  /** 'Transform' toolbar button. */
  JButton  editToolTransformBtn;
  /** 'H Mirror' toolbar button. */
  JButton  editToolHMirrorBtn;
  /** 'V Mirror' toolbar button. */
  JButton  editToolVMirrorBtn;
  /** 'Align Left' toolbar button. */
  JButton  editToolAlignLeftBtn;
  /** 'Align Top' toolbar button. */
  JButton  editToolAlignTopBtn;
  /** 'Align Right' toolbar button. */
  JButton  editToolAlignRightBtn;
  /** 'Align Bottom' toolbar button. */
  JButton  editToolAlignBottomBtn;
  ...
  public JDrawEditorPanel() {
    ...
    // Views -----------------------------------------
    hideshowDeviceTreeMenuItem = new JMenuItem("Show device tree");
    hideshowDeviceTreeMenuItem.addActionListener(this);
    ...
    viewsMenu.add(hideshowDeviceTreeMenuItem);
    ...
    add(theMenu,BorderLayout.NORTH);

    treeSplitPane.setRightComponent(editorPanel);
  ...
  public void setPlayer(JDrawEditor editor) {
    ...
    playerSplitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT);
    ...
    playerSplitPane.add(thePlayer);
    playerSplitPane.add(fpTextView);
    framePlayer.getContentPane().add(playerSplitPane,BorderLayout.CENTER);
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (src==editToolPasteBtn || src==editPasteMenuItem) {
    ...
    } else if (src==hideshowDeviceTreeMenuItem ) {

      if( treeSplitPane.getLeftComponent()==null ) {
        if(deviceTree==null) deviceTree = new JDDeviceTree();
        treeSplitPane.setLeftComponent(deviceTree);
        hideshowDeviceTreeMenuItem.setText("Hide device tree");
        treeSplitPane.revalidate();
      } else {
        treeSplitPane.setLeftComponent(null);
        hideshowDeviceTreeMenuItem.setText("show device tree");
        treeSplitPane.revalidate();
      }
      ...
      playerSplitPane.setDividerLocation(playerSplitPane.getHeight()-80);
      playerSplitPane.revalidate();
  ...
  public static void main(String[] args) {
    ...
    final JDrawEditorPanel jdp = new JDrawEditorPanel();
    jdp.setEditor(ed);
    jdp.setPlayer(py);
    jdp.setLibraries(null);
    if(args.length==1) {
      try {
        ed.loadFile(args[0]);
      } catch (IOException e) {
