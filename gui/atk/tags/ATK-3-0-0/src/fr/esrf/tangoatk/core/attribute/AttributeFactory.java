  ...
  private EnumScalar getEnumScalar(Device device,AttributeInfo config)
  {
	 ...
	 if (dbAtt == null)
	 {
	    ...
	    else
	    {
		   ...
		   {
		      enumLabs = EnumScalar.getNonEmptyLabels(propVal[0].split("\n"));
		   }
	       }
	    }
      }
	 ...
	 if (dbAtt == null)
	 {
	    ...
	    {
	       //System.out.println("EnumSetExclusion property not found for "+name);
	    }
	    else
	    {
		  ...
		  // System.out.println("propVal = null");
		   ...
		   if (propVal[0].length() > 2)
		   {
		      enumSetExcludeLabs = EnumScalar.getNonEmptyLabels(propVal[0].split("\n"));
		   }
	       }
	    }
      }
