...
//    if (skippingRefresh) return;
  ...
  public void loadAttProperties()
  {
     ...
     try
     {
         ...
         if (!dbAtt.is_empty(XMIN_ATT_PROP))
         {
             propDbDatum = dbAtt.datum(XMIN_ATT_PROP);
             if (propDbDatum != null)
                if (!propDbDatum.is_empty())
                       xmin_att_name = propDbDatum.extractString();
         }
          
         if (!dbAtt.is_empty(XMAX_ATT_PROP))
         {
             propDbDatum = dbAtt.datum(XMAX_ATT_PROP);
             if (propDbDatum != null)
                if (!propDbDatum.is_empty())
         ...
         if (!dbAtt.is_empty(XMIN_PROP))
         {
             propDbDatum = dbAtt.datum(XMIN_PROP);
             if (propDbDatum != null)
                if (!propDbDatum.is_empty())
                {
                   hasXmin = true;
         ...
         if (!dbAtt.is_empty(XMAX_PROP))
         {
             propDbDatum = dbAtt.datum(XMAX_PROP);
             if (propDbDatum != null)
                if (!propDbDatum.is_empty())
                {
                   hasXmax = true;
