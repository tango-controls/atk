...
core-release:
	(cvs tag $(CORE-RELEASE) configure                     \
                                 ...
                                 .core-build                   \
                                 Makefile &&                   \
         cd src/fr/esrf/tangoatk/core &&                       \
         cvs tag $(CORE-RELEASE))
...
widget-release:
	(cvs tag $(WIDGET-RELEASE) .widget-build  &&           \
	 cd src/fr/esrf/tangoatk/widget           &&           \
         cvs tag -R $(WIDGET-RELEASE))
...
core-build:
	(cd $(BUILDDIR);                                                    \
         cvs -d $(CVSROOT) export -r  $(CORE-RELEASE) jclient_framework;    \
	(cd jclient_framework/ATK; export JAVA_HOME=$(JAVA_HOME) &&         \
         ./configure && make core && make core-test && make core-jar))

widget-build:
	(cd $(BUILDDIR);	                                            \
         cvs -d $(CVSROOT) export -r  $(WIDGET-RELEASE) jclient_framework;  \
         && ./configure && make widget                                      \
         && make widget-test && make widget-jar))
