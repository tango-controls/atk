...
public class WheelSwitch extends JComponent {
  ...
  private static Color selectionColor = new Color(156, 154, 206);
    ...
    if( !editMode && nbButton>0 ) {
      switch(code) {
        case KeyEvent.VK_RIGHT:
          selButton++;
          if( selButton>=nbButton ) selButton=nbButton-1;
          updateButtonFocus();
          repaint();
          break;
        case KeyEvent.VK_LEFT:
          selButton--;
          if( selButton<0 ) selButton=0;
          updateButtonFocus();
          repaint();
          break;
        case KeyEvent.VK_UP:
  ...
  synchronized public void setPrecision(int inb, int fnb) {
  ...
  private void computeDigitSize() {
    ...
    if (dz == null) {
      ...
      for (i = 0; i < charSet.length; i++) {
        ...
        Dimension b = ATKGraphicsUtils.measureString(charSet[i], getFont());
        int w = b.width + 1;
        if (w > max_width) max_width = w;

      }

      max_height = (int) ATKGraphicsUtils.getLineMetrics("0123456789", getFont()).getAscent() + 1;
