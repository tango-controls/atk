...
/*  Removed this part of code because it makes impossible the use of #dabse=no. */
...
*/
	...
	idlv = get_idl_version();
        trace(DeviceFactory.TRACE_SUCCESS , "Device.get_idl_version("+name+") ok" , t0);
    ...
    /* Removed the following lines in order to support events ONLY for IDL 3
       device servers and higher .... decision made in SEG meeting
       14/01/2005. So we do not support events anymore for the device servers
       event compatible but with device impl 2.
      
    // for lower IDL versions check event compatibility in another manner
    if (supportsEvents == false) 
    {
	try
	{
	  String        admName = null;
	  DeviceProxy   admDev = null;
        ...
        trace(DeviceFactory.TRACE_SUCCESS , "Will call Device.adm_name("+name+") .... " , t0);
	  admName = adm_name();
  ...
  public void periodic (TangoPeriodicEvent e) 
  {
    ...
    if (da != null) {
      ...
      try {
        ...
        } else if (da.getName().equals("status")) {
          newStatus = null;
