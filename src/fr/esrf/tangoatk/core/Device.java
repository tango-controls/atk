...
//duree = after -  before;
//System.out.println("getState+getStatus+fireEventChanges for device : "+ name + " took : " + duree + " milli seconds.");
  }
...
//	DeviceData  d=null;
    try {
//before = System.currentTimeMillis();
      current_state = this.state(ApiDefs.FROM_CMD);
//after = System.currentTimeMillis();
//long duree = after -  before;
//System.out.println("State for device : "+ name + " took : " + duree + " milli seconds.");
//          d = this.command_inout("State");
//	    current_state = d.extractDevState();
    } catch (DevFailed dev) {
//after = System.currentTimeMillis();
//long duree = after -  before;
//System.out.println("Failed (DevFailed) to get the device state for : "+ this.getName() + dev);
//System.out.println("State for device : "+ name + " took : " + duree + " milli seconds.");
//fr.esrf.TangoDs.Except.print_exception(dev);
      deviceError("Couldn't read state: ", new ConnectionException(dev));
      return IDevice.UNKNOWN;
    } catch (Exception e) {
//System.out.println("Failed (Exception) to get the device state for : "+ this.getName());
...
//long before, after;
    try {
//before = System.currentTimeMillis();
      current_status = this.status(ApiDefs.FROM_CMD);
      ;
//after = System.currentTimeMillis();
//long duree = after -  before;
//System.out.println("Status for device : "+ name + " took : " + duree + " milli seconds.");
//            d = this.command_inout("Status");
//	    return(d.extractString());
    } catch (Exception e) {
