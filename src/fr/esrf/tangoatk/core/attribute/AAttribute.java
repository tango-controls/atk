...
public abstract class AAttribute implements IAttribute {
	...
	ATKLogger.getInstance(AAttribute.class.getName());
    transient protected Logger attributeLog;
    protected Device device;
    transient protected AttributeInfo config;
    transient protected DeviceAttribute attribute;
	...
	attributeLog = Logger.getLogger(getName().replace('/', '.'));
	log.debug("created attributeLogger");
	Log4jConfigurator.addLogger(attributeLog);
	try {
    ...
    public String getFormat() {
	    ...
	    (AttrDataFormat)getProperty("format").getValue();
	switch (format.value()) {
	case AttrDataFormat._SCALAR:
	    return "scalar";
	case AttrDataFormat._SPECTRUM:
	...
	timer.startTimer(Thread.currentThread());
	String s = getProperty("unit").getValue().toString();
	timer.startTimer(Thread.currentThread());
	return s;
    }
    ...
    protected final DeviceAttribute readValueFromNetwork() throws fr.esrf.Tango.DevFailed {
	attribute = device.quickReadAttribute(nameSansDevice);
	TimeVal t = attribute.getTimeVal();			       
	timeStamp  = ((t.tv_sec * 1000L) +
		      ((t.tv_usec / 1000L)));
	...
	AttrQuality q = attribute.getAttributeValueObject().quality;
