...
public class AttributeFactory extends AEntityFactory {
  ...
  private DevStateSpectrum getDevStateSpectrum(Device device, AttributeInfoEx config)
  {
     String         name = config.name;
     ...
     DevStateSpectrum  stateSpect = null;
     String[]          stateLabels = null;

     stateSpect = new DevStateSpectrum();
     try
     {
         ...
         if (dbAtt != null)
         {
             ...
             {
                 //System.out.println("Found " + IDevStateSpectrum.STATE_LABELS + " property for "+name);
                 propDbDatum = dbAtt.datum(IDevStateSpectrum.STATE_LABELS);
                 //System.out.println(IDevStateSpectrum.STATE_LABELS+" = "+propVal);
                 if (propDbDatum != null)
                    if (!propDbDatum.is_empty())
                       stateLabels = propDbDatum.extractStringArray();
                 
                 if ((stateLabels!=null) && (stateLabels.length > 0))
                     stateSpect.setStateLabels(stateLabels);
             }
          }        
          // Check if the colors should be inverted for open and close states
          if (!dbAtt.is_empty(Device.OPEN_CLOSE_PROP))
          {
              //System.out.println("Found " + Device.OPEN_CLOSE_PROP + " property for "+name);
              propDbDatum = dbAtt.datum(Device.OPEN_CLOSE_PROP);
              //System.out.println(Device.OPEN_CLOSE_PROP+" = "+propVal);
              if (propDbDatum != null)
                 if (!propDbDatum.is_empty())
                    invertedOpenCloseLogic = propDbDatum.extractStringArray();
              if ( (invertedOpenCloseLogic != null) && (invertedOpenCloseLogic.length > 0) )
                  stateSpect.setInvertedOpenClose(invertedOpenCloseLogic);
          }
           // Check if the colors should be inverted for open and close states
          if (!dbAtt.is_empty(Device.INSERT_EXTRACT_PROP))
          {
              //System.out.println("Found " + Device.INSERT_EXTRACT_PROP + " property for "+name);
              propDbDatum = dbAtt.datum(Device.INSERT_EXTRACT_PROP);
              //System.out.println(Device.INSERT_EXTRACT_PROP+" = "+propVal);
              if (propDbDatum != null)
                 if (!propDbDatum.is_empty())
              ...
              if ( (invertedInsertExtractLogic != null) && (invertedInsertExtractLogic.length > 0) )
                  stateSpect.setInvertedInsertExtract(invertedInsertExtractLogic);
          }
     }
     catch (Exception ex)
     {
  ...
  private DevStateScalar getDevStateScalar(Device device, AttributeInfoEx config)
  {
     String         name = config.name;
     DbAttribute    dbAtt=null;
     DbDatum        propDbDatum=null;
 
     boolean        invertedOpenCloseLogic = false;
     boolean        invertedInsertExtractLogic = false;
     DevStateScalar stateScalar = null;

     stateScalar = new DevStateScalar();
     try
     {
         dbAtt = device.get_attribute_property(name);
         if (dbAtt != null)
         {
             ...
             if (!dbAtt.is_empty(Device.OPEN_CLOSE_PROP))
             {
                 ...
                 if (propDbDatum != null)
                    if (!propDbDatum.is_empty())
                       invertedOpenCloseLogic = propDbDatum.extractBoolean();
                 if (invertedOpenCloseLogic == true)
                     stateScalar.setInvertedOpenClose(true);
             }
             ...
             if (!dbAtt.is_empty(Device.INSERT_EXTRACT_PROP))
             {
                 ...
                 if (propDbDatum != null)
                    if (!propDbDatum.is_empty())
                       invertedInsertExtractLogic = propDbDatum.extractBoolean();
                 if (invertedInsertExtractLogic == true)
                     stateScalar.setInvertedInsertExtract(true);
             }
        }        
     }
     catch (Exception ex)
  ...
  private AAttribute getScalar(Device device,AttributeInfoEx config) {
    ...
    switch (dataType) {
      ...
      case Tango_DEV_STATE:
            dss = getDevStateScalar(device, config);
  ...
  private AAttribute getSpectrum(Device device,AttributeInfoEx config) {
    ...
    switch (dataType) {
      ...
      case Tango_DEV_BOOLEAN:
        ...
        return bs;
      case Tango_DEV_STATE:
        dss = getDevStateSpectrum(device, config);
        return dss;
      default:
